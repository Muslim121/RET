<?php // Example 09: members.php
  ...
  if (isset($_GET['view']))
  {
    ...
    echo "<a data-role='button' data-transition='slide'
          href='messages.php?view=$view&r=$randstr'>Написать $name сообщение</a>";
    die("</div></body></html>");
  }
  ...
  while ($row = $result->fetch())
  {
    ...
    elseif ($t1)         echo " &larr; Вы подписаны на этого пользователя";
    ...
    if (!$t1) echo " [<a data-transition='slide'
      ...
      href='members.php?remove=" . $row['user'] . "&r=$randstr'>Удалить</a>]";
  }
